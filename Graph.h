...
#undef minor
	...
	*/
	...
	struct Box : Shape { //Basic Requirements 1
		Box(Point xy, int ww, int hh, int rr) :w{ ww }, h{ hh }, r{ rr } //getting radius
		{
			add(xy);
		}
		Box(Point x, Point y) :w{ y.x - x.x }, h{ y.y - x.y }//first point
		{
			add(x);
		}
	...
	struct Regular_polygon : Shape {	// Basic Requirements 2
		Regular_polygon(Point p, int nn, int rr) : n{ nn }, r{ rr } //center,number of sides, distance from coner
		{
			add(p);
		}
	...
	struct Star : Shape {	// Basic Requirements 3
		Star(Point p, int nn, int rr1, int rr2) : n{ nn }, r1{ rr1 }, r2{ rr2 } //center,number of sides, radius1, radius2
		{
			add(p);
		}
		...
		Point center() const { return { point(0).x + r1, point(0).y + r1 }; }
